...
do
    echo "$i/$n_lines: $line"
    result=$(/usr/bin/time -v timeout 1200 python static_analyser.py --app $line -l $GET_N_FUNCTIONS_FOUND -v t -d f --csv f --analyse-linker t --user-input Y 2>&1 | sed -n -e '/^Total number of syscalls:/p' -e '/Elapsed (wall clock) time/p' | awk '{print $NF}' | awk 'BEGIN {RS=""; FS="\n"} { if (NF == 2) {printf("%s\telapsed time: %s", $1, $2)} else {printf("timeout\telapsed time: %s", $1)} }')
